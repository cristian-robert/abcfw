...
public enum CallableFeatureMethod implements CallableMethod {
    ...
    $PAYLOAD_VALUE("Returns payload value by given key.", "$PAYLOAD_VALUE(Company_CSR)") {
        public String execute(final List<String> args, final ScenarioContext scenarioContext) {
            assertArgsCountEquals(args, 1);
            final String payloadKey = args.get(0);
            final Map<String, PayloadValue> payloadValues = scenarioContext.getPayloadValues();
    ...
    $SYS_DATE_OF_FORMAT("Generates sys date of given format") {
        public String execute(List<String> args, final ScenarioContext scenarioContext) {
            assertArgsCountEquals(args, 1);
    ...
    $LAST_MATCHED_RECORD("Returns the value of the last matched record node key passed as argument") {
        public String execute(List<String> args, final ScenarioContext scenarioContext) {
            assertArgsCountEquals(args, 1);
            ObjectNode lastMatchedRecord = scenarioContext.get(ScenarioContextKeys.LAST_MATCHED_RECORD, ObjectNode.class);
    ...
    $RANDOM_TEXT_OF_LENGTH("Generates a random text of given <stringLength> separated into groups by <groupSize>",
        ...
        public String execute(List<String> args, final ScenarioContext scenarioContext) {
            ...
            String input = CallableFeatureMethod.$RANDOM_STRING_OF_LENGTH.execute(Collections.singletonList(stringLength), scenarioContext);
