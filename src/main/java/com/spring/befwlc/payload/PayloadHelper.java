...
public class PayloadHelper {
    ...
    public static String generateValue(final String inputValue) {
        if (inputValue != null) {
            if (valueMatchesPattern(inputValue, CALLABLE_FEATURE_METHOD_REGEX)) {
                ...
                for (final String arg : argList) {
                    dynamicMethodArgs.add(generateValue(arg.trim()));
                }
                return CallableFeatureMethodSelector.executeCallableMethod(methodName, dynamicMethodArgs);
                ...
                if (valueMatchesPattern(inputValue, STATIC_FEATURE_METHOD_NAME_REGEX)) {
                    return CallableFeatureMethodSelector.executeStaticMethod(inputValue);
                }
